...
let usage_msg =
  "resspec <source-dir-or-file> -o <output-filename> -t <tag1>,...,<tagN> -i"
;;
...
let speclist =
  ...
  ; ( "-l"
    , Arg.Set_int level
    , "Set your current level (number of skill points in interactive mode)." )
  ]
;;
...
let () =
  ...
  then (
    ...
    let () = builder_call tagset rm !output_file in
    Printf.printf
